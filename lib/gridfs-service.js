...
GridFSService.prototype.connect = function (cb) {
  ...
  if (!this.db) {
    ...
    MongoClient.connect(url, self.options, (error, client) => {
      if (!error) {
        if (self.options.database !== null && self.options.database !== undefined) {
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    var options = {
      ...
      mode: 'w'
		...
		var gridfs = new GridFSBucket(self.db);
		var stream = gridfs.openUploadStreamWithId(options._id, options.filename, {
...
GridFSService.prototype.uploadStream = function (containerName, streamObj, options, cb) {
  var self = this;
  ...
  streamObj.pipe(stream);
};

GridFSService.prototype.getUploadStream = function (containerName, streamObj, options, cb) {
  var self = this;
 ...
 */
GridFSService.prototype.download = function (containerName, fileId, res, cb, options) {
  var self = this;
  ...
  collection.find({
    ...
    if (error) {
      return cb(error);
		...
		var gridfs = new GridFSBucket(self.db);
		var stream = gridfs.openDownloadStream(file._id);

    // set headers
    res.set('Content-Type', file.metadata.mimetype);
    res.set('Content-Length', file.length);
    if (options !== undefined && options !== null && options.ignoreDisposition === true) {
    } else {
...
GridFSService.prototype.downloadStream = function (containerName, fileId) {
  var self = this;
  ...
  return new Promise((resolve, reject) => {
    ...
    collection.find({
      ...
      stream['fd']= true;
      stream.start = 0;
      stream.end = file.length;
      stream.knownLength = file.length;
      stream.contentType = file.metadata.mimetype;
...
GridFSService.prototype.downloadContainer = function (containerName, req, res, cb) {
  ...
  collection.find({
    ...
    function next() {
      if (files.length > 0) {
        var file = files.pop();
				var fileStream = gridfs.openDownloadStream(file._id);
 ...
 * Download middleware for the HTTP request.
 */
GridFSService.prototype.downloadInline = function (containerName, fileId, res, cb) {
  var self = this;
...
GridFSService.prototype.downloadInline.http = {
  verb: 'get',
  path: '/:containerName/downloadInline/:fileId'
};
